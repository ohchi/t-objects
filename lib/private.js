...
/*
 * Get constructor name for a object.
 * It doesn't work for elementary values in nodejs.
 * It may not work in old browsers.
 * 
 * @param value	Some class instance.
 * @return		Constructor name.
 */
function typeOfObject(value) { return Object.getPrototypeOf(value).constructor.name; }
...
function compile(template, tpath){
	...
	for (var i in template) {
		var e = template[i];
		if (_.isObject(e) && typeOfObject(e) == 'Object') {
...
function callFn(context, depsfn){
	if (_.isFunction(depsfn)) {
		...
		return fn.apply(context, _.map(deps, function(e){ return context[e] }));
	}
}
 ...
 * @param constructor	Values collection constructor.
...
function addVariety(obj, keyval, constructor){
	...
	function _expand(e){
		var arr = callFn(e, constructor);
		if (_.isUndefined(arr) || (_.isObject(arr) && _.size(arr) == 0)) return e;
		if (!_.isObject(arr) ||
			(!_.isArray(arr) && typeOfObject(arr) != 'Object')) {
	...
	if (typeOfObject(obj) == 'Object') {
		if (_.isUndefined(callFn(obj, constructor))) {
			return obj;
		} else {
	...
	try {
		...
		for (var i=0; i<q.length; i++) {
			...
			if (_.isArray(e)) {
				...
				obj = addVariety(obj, e, tobj[key]);
			} else if (isFn(tobj[e])) {
				// Property constructor
				// case 2
				addProperty(obj, e, tobj[e]);
			} else if (_.isObject(tobj[e])) {
				if (typeOfObject(tobj[e]) == 'Object' && tobj[e]['-->']) {
					...
					if (_.isArray(obj)) {
						...
						for (var j=0; j<obj.length; j++) {
							var _e = obj[j];
							_e[e] = build(tobj[e], data, _e, root, e);
						}
					} else {
						// A single object
						// case 3_2
						obj[e] = build(tobj[e], data, obj, root, e);
					}
				} else {
					// Constant proprety
					// case 4
					addConstant(obj, e, tobj[e]);
				}
			} else if (tobj[e] == ':external') {
				// External property
				// case 5
				addConstant(obj, e, data[e]);
			} else {
				// Constant proprety
				// case 6
				addConstant(obj, e, tobj[e]);
			...
			if (e == '?') {
				...
				if (_.isArray(obj)) {
					...
					return postBuild(obj, tobj);
				} else {
					if (!qq(obj)) return null;
				}
			}
		}
...
function clean(obj){
	...
	function cln(obj, inx){
		var o = clean(obj[inx]);
		if (o === null) delete obj[inx];
		else obj[inx] = o;
	}
	
	if (_.isObject(obj) && typeOfObject(obj) == 'Object') {
		if (obj['->']) {
			if (!_.isUndefined(obj['?']) && !obj['?']) return null;
			delete obj['->'];
			delete obj['-->'];
			delete obj[':'];
			delete obj['?'];
		...
		return _.filter(obj, function(e){ return e != null });
	}
